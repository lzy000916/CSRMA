    ...
    for i, pack in enumerate(train_loader, start=1):
        ...
        if i % 20 == 0 or i == total_step:
            print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], '
                  ' lateral-5: {:0.4f}], lateral-4: {:0.4f}], lateral-3: {:0.4f}], lateral-2: {:0.4f}], lateral-1: {:0.4f}], lateral-0: {:0.4f}]'.
                  format(datetime.now(), epoch, opt.epoch, i, total_step,
                         loss_record5.show(), loss_record4.show(), loss_record3.show(), loss_record2.show(), loss_record1.show(), loss_record0.show()))
...
if __name__ == '__main__':
    ...
    # ---- build models ----
    # torch.cuda.set_device(0)  # set ur gpu device, if you run by .sh, it is can comment.
    A = [CSRMAUNet(1, 2)]
    num = -1
    ...
    lrs = CosineAnnealingLR(optimizer, T_max=40, verbose=True)
